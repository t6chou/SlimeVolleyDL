...
class SlimeVolleyEnv(gym.Env):
  ...
  def step(self, action, otherAction=None):
    ...
    self.t += 1
    # print("***** slimevolleyenv.setAction:action=", action, "-- pixel:", self.from_pixels , " -- atari:", self.atari_mode)
    ...
    if self.atari_mode:
      ...
      # print("*** discreteToBox(action)=", action)
      if otherAction is not None:
        # print("*** atari:yesotherAction:discreteToBox(otherAction)=", otherAction)
        otherAction = self.discreteToBox(otherAction)
      else:
        obs = self.game.agent_left.getObservation()
        otherAction = self.policy.predict(obs)
        otherAction = self.discreteToBox(otherAction)
        # print("*** atari:noOtherAction: discreteToBox(otherAction)=", otherAction)

    if otherAction is None: # override baseline policy
      obs = self.game.agent_left.getObservation()
      otherAction = self.policy.predict(obs)
      # print("*** noAtari:noOtherAction: discreteToBox(otherAction)=", otherAction)
...
''' New Environment added which has no pixel input but does have atari action outputs.
    This allows for use with DQN using MlpPolicy.
'''
class SlimeVolleyAtariNoPixelsEnv(SlimeVolleyEnv):
  from_pixels = False
  atari_mode = True
...
register(
    id='SlimeVolleyNoPixelAtariActions-v0',
    entry_point='slimevolleygym.slimevolley:SlimeVolleyAtariNoPixelsEnv'
)
